// src/database/repos/rpostShard.js
// Rpost shard tracker config stored in rpost_shard_config table.
...
async function ensureSchema() {
  await run(`CREATE TABLE IF NOT EXISTS rpost_shard_config (
  ...
  await run(`ALTER TABLE rpost_shard_config ADD COLUMN last_run_at INTEGER DEFAULT 0`).catch(() => {});
  await run(`ALTER TABLE rpost_shard_config ADD COLUMN previous_message_id TEXT`).catch(() => {});
}

function getRpostShardConfig(guildId) {
  return get(`SELECT * FROM rpost_shard_config WHERE guild_id = ?`, [guildId]);
}

function upsertRpostShardConfig({
  guild_id,
  channel_id = null,
  ...
  return new Promise((resolve, reject) => {
    ...
    db.run(`INSERT OR IGNORE INTO rpost_shard_config (guild_id) VALUES (?)`, [guild_id], (insErr) => {
      ...
      vals.push(guild_id);
      db.run(
        `UPDATE rpost_shard_config SET ${sets.join(', ')} WHERE guild_id = ?`,
        vals,
