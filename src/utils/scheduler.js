...
// NEW: import cleanup helpers from dashboards
const {
  cleanupWallStatusMessages,
} = require('../discord/dashboards/wallCheckBoard');
const {
  cleanupOutpostStatusMessages,
} = require('../discord/dashboards/outpostBoard');
  ...
  for (const [guildId, guild] of client.guilds.cache) {
    try {
      ...
      if (wcfg?.channel_id && !gcfg?.base_alerts_paused) {
        const channel = guild.channels.cache.get(wcfg.channel_id) || await guild.channels.fetch(wcfg.channel_id).catch(() => null);
        if (channel) {
          ...
          if (overdue && !alreadyNotified) {
            // Clean up previous overdue + clear messages for buffer/walls
            try {
      ...
      if (ocfg?.channel_id && !gcfg?.outpost_alerts_paused) {
        const channel = guild.channels.cache.get(ocfg.channel_id) || await guild.channels.fetch(ocfg.channel_id).catch(() => null);
        if (channel) {
          ...
          if (overdue && !alreadyNotified) {
            // Clean up previous overdue + clear messages for outpost
            try {
